...
class pascal_voc(imdb):
    ...
    def view_fliter(self, poses, views, distance):
        ...
        for ix in range(len(poses)):
            ...
            views[ix], distance[ix] = self.view_label12(azimuth,elevation)
        ...
        for jx in range(len(e_intervals)) :
            ...
            for ix in range(len(a_intervals)) :
                ...
                if ix % max_alab != 0 or ix==0: 
                    ...
                    distance[jx*4 + ix] = getDistance(mid_e,mid_a,elevation,azimuth)
                else:
                    temp = getDistance(mid_e,mid_a,elevation,azimuth)
                    distance[jx*4 + ix%4] = (temp+distance[jx*4 + ix%4])/2
        #pdb.set_trace()
        # label calculate function

        try:
            view_label = e_label*(4) + a_label+1
            if view_label > 13 :
                pdb.set_trace()
        except:
            pdb.set_trace()
