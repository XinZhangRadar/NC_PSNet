...
class _fasterRCNN(nn.Module):
    ...
    def __init__(self, classes, class_agnostic):
        ...
        self.view_size_fg = 26
    
        self.view_size = self.view_size_fg+1
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes,distance):
        ...
        distance_gt = distance.data[:,:-1].cuda()
        dis_score = self.Gauss(distance_gt)
        dis_score = dis_score.cuda()
        _,dis_index = dis_score.sort()
        
        #dis_score = torch.div(dis_score,dis_score.sum(1).view(dis_score.shape[0],1)).cuda()
        ...
        if self.training:
            ...
            #distance_gt  = Variable(distance_gt.view(-1, distance_gt.size(2)))
            #dis_score = Variable(torch.from_numpy(np.zeros((distance_gt[:,:-1].shape[0],self.view_size_fg))).float(),requires_grad=False)
        ...
        if self.if_show:
            ...
            self.draw_features(1,self.view_size_fg,bb,'view1.jpg')
        ...
        for batch in range(self.batch_size):
            vs_weight.append(dis_score[batch].view(1,-1).repeat(int(pooled_feat.shape[0]/self.batch_size),1))
        vs_weight = torch.cat(vs_weight,0)



        #dis_score = torch.cat((dis_score[0].repeat(int(pooled_feat.shape[0]/self.batch_size),1),dis_score[1].repeat(int(pooled_feat.shape[0]/self.batch_size),1)),0)
        
        pooled_light =  torch.mul(pooled_feat.view(*pooled_feat.shape[:2],-1).permute(2,0,1),vs_weight).permute(1,2,0).sum(1).view(-1,pooled_feat.shape[2],pooled_feat.shape[3],pooled_feat.shape[4])

        
        
        #pooled_feat = pooled_feat.view(self.batch_size,int(pooled_feat.shape[0]/self.batch_size), self.view_size_fg, self.k, cfg.POOLING_SIZE, cfg.POOLING_SIZE)
        #dis_score = dis_score.view(self.batch_size,1,self.view_size_fg,1,1,1)
        #pooled_light = (pooled_feat * dis_score.expand_as(pooled_feat)).view(-1,self.view_size_fg, self.k, cfg.POOLING_SIZE, cfg.POOLING_SIZE)

        if self.if_show:
            vs_roi = torch.mul(pooled_feat.view(*pooled_feat.shape[:2],-1).permute(2,0,1),vs_weight).permute(1,2,0).view(-1,pooled_feat.shape[1],pooled_feat.shape[2],pooled_feat.shape[3],pooled_feat.shape[4])

            aa = vs_roi[0].sum(1).unsqueeze(0).detach().cpu().numpy()
            self.draw_features(1,self.view_size_fg,aa,'view_roi.jpg')
        #import pdb;pdb.set_trace()


        #pooled_light = pooled_light.sum(1)
        #pooled_light = pooled_feat.sum(1)
        #roi_response = self.view_response(pooled_feat)
        #dis_score_pred = F.softmax(roi_response,dim=1)
        #vs_weight = dis_score_pred
        '''
        b, v, l, _, _  = pooled_feat.size()

        pooled_light = pooled_feat * vs_weight.view(b, v, 1, 1, 1).expand_as(pooled_feat)

        pooled_light = pooled_light.sum(1)
        '''
        #pooled_light =  torch.mul(pooled_feat.view(*pooled_feat.shape[:2],-1).permute(2,0,1),dis_score).permute(1,2,0).sum(1).view(-1,pooled_feat.shape[2],pooled_feat.shape[3],pooled_feat.shape[4])
        ...
        cls_score,bbox_pred = self.RCNN_Net(pooled_light)
        ...
        if self.training:
            ...
            #RCNN_loss_select = F.cross_entropy(roi_response_fg, roi_v_label[index_fg]-1) 
