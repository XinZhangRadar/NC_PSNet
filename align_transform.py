...
class Align():
    ...
    def warp_image(self, source, target, M):
        ...
        # Show the result
        #cv2.imshow('img1', merge)
        #cv2.imshow('img2', warp)
        #cv2.imshow('img3', target)
        #cv2.imshow('img4', source)

        cv2.waitKey(0)
        cv2.destroyAllWindows()

        return warp,target,merge
    ...
    def align_image(self):
        ...
        img_source = self.img1
        img_target = self.img2
        ...
        warp,target,merge = self.warp_image(img_source, img_target, M)

        #return warp,target,M
        return warp,merge,M
